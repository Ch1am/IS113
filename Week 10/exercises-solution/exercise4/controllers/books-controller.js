...
// bookList function is to edit-book it is similar to book-display but it has an edit button
...
// this function gets the ISBN number from the url and 
// retrieve it to render on show-a-book.ejs
exports.getMarkedBook = async (req, res) => {
  const isbnNo = req.query.isbn; // Get isbn from url
  console.log(isbnNo);
  ...
  try {
    ...
    res.render("show-a-book", {result:result ||null}); 
  } catch (error) {
    console.error(error);
...
// this function will retrieve the ISBN number from the hidden field 
// in show-a-book.ejs and do the actual deletion of the document
exports.deleteABook = async (req, res) => {
  const isbnNo= req.body.isbn; //in the hidden field.
  console.log(isbnNo);
  try {
    let success= await Book.deleteBook(isbnNo);
